...
token_tmp = os.path.join(config_root, 'KIS_token_' + datetime.today().strftime("%Y%m%d"))  # 토큰 로컬저장시 파일명 년월일
...
# product='XX' 변경하세요 (계좌번호 뒤 2자리)
def auth(product=_cfg['my_prod'], url=None):
    p = {
        "grant_type": "client_credentials",
    }
    # 개인 환경파일 "kis_devlp.yaml" 파일을 참조하여 앱키, 앱시크리트 정보 가져오기
    # 개인 환경파일명과 위치는 고객님만 아는 위치로 설정 바랍니다.
    if _cfg['trading_mode'] == 'real':  # 실전투자
        svr = 'prod'
        ak1 = 'my_app'  # 앱키 (실전투자용)
        ak2 = 'my_sec'  # 앱시크리트 (실전투자용)
    elif _cfg['trading_mode'] == 'mock':  # 모의투자
        svr = 'vps'
        ak1 = 'paper_app'  # 앱키 (모의투자용)
        ak2 = 'paper_sec'  # 앱시크리트 (모의투자용)
    else:
        raise ValueError(f"Invalid trading_mode: {_cfg['trading_mode']}")
...
def reAuth(product=_cfg['my_prod']):
    if _cfg['trading_mode'] == 'real':
        svr = 'prod'
    elif _cfg['trading_mode'] == 'mock':
        svr = 'vps'
    else:
        raise ValueError(f"Invalid trading_mode: {_cfg['trading_mode']}")
    n2 = datetime.now()
